...
fn main() {
    App::new()
        ...
        .add_systems(Update, ai.run_if(in_state(GamePhase::Ai)))
        .run();
}
...
fn setup(
    ...
    commands.insert_resource(Scores { player: 0, ai: 0 });
...
fn compute_hand_total(hand_query: &Query<(Entity, &Sprite), With<HandDie>>) -> usize {
    hand_query
        .iter()
...
fn ai(
    ...
    if timer.0.just_finished() {
        let hand_total = compute_hand_total(&hand_query);
        if hand_total < 20 && scores.ai + hand_total < 100 {
            let new_roll = rng.0.range(1..=6);
            if new_roll == 1 {
                clear_die(&hand_query, &mut commands);
            ...
            scores.ai += hand_total;
            state.set(GamePhase::Player);
            for (entity, _) in hand_query {
